...
const KEY_BINDINGS: &[(&str, &str)] = &[
    ...
    (
        "Ctrl-T",
        "Toggle mouse capture (click-to-expand vs text select)",
    ),
    ("Ctrl-D", "Quit"),
];
...
fn open_mcp_picker(app: &mut App) {
    ...
    if servers.is_empty() {
        return app
            .note("no MCP servers configured (add them in the desktop app or mcp_config.json)");
    }
    let items = servers
        .into_iter()
        .map(|s| PickerItem {
...
async fn load_thread(app: &mut App, thread: &serde_json::Value) {
    ...
    for msg in &messages {
        ...
        if role == "user" {
            // Skill/command invocations persisted their full prompt template as
            // the user message (that is what the model sees on continuation),
...
mod tests {
    ...
    use super::{
        ...
        clipboard_path, compact_tokens, diff_lines, finish_login, finish_update_install,
        group_activity, group_detail_lines, group_summary, handle_ask_key, handle_ask_mouse,
        handle_ask_paste, handle_key, handle_mouse, image_mime, input_content_lines,
        load_image_file, message_text, note_update, open_config_screen, parse_command,
        partial_json_field, restore_goal, restore_run_mode, restore_todos, rewind_to, run_command,
        running_group_row, split_reasoning, starting_call_lines, strip_system_xml_tags,
        subagent_activity, subagent_name_from_run_id, summarize_result, tilde_path,
        tokens_per_second, tool_activity, tool_finished, transcript_top_padding,
        unescape_partial_json_string, user_content_parts, App, CurrentRun, Pending, PendingImage,
        PickerKind, ResumeTarget, Row, SnapshotJob, Status, AGENT_SETTINGS, DIFF_ADD_BG,
        DIFF_DEL_BG, DIFF_MAX_ROWS, DIFF_PREVIEW_MAX_ROWS, KEY_BINDINGS, SLASH_COMMANDS, SPINNER,
        SPINNER_ADVANCE_MS,
    };
